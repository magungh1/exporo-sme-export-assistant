...
from .config import DEFAULT_EXTRACTED_DATA, EXPORT_READINESS_PROMPT
...
def check_embedding_availability():
    ...
    if EMBEDDING_AVAILABLE is None:
        try:
            # These imports are only for availability checking
            import importlib.util

            faiss_spec = importlib.util.find_spec("faiss")
            numpy_spec = importlib.util.find_spec("numpy")
            transformers_spec = importlib.util.find_spec("sentence_transformers")

            if faiss_spec and numpy_spec and transformers_spec:
                EMBEDDING_AVAILABLE = True
            else:
                EMBEDDING_AVAILABLE = False
                st.warning(
                    "⚠️ Text embedding libraries not available. Some features may be limited."
                )
        except Exception:
            EMBEDDING_AVAILABLE = False
            st.warning(
...
def display_memory_bot_data():
    ...
    with col1:
        ...
        st.write(
            "**Product:** "
            + memory_data.get("product_details", {}).get("name", "Not specified")
        )
        st.write(
            "**Category:** " + memory_data.get("product_category", "Not specified")
        )
    ...
    if st.button("✏️ Update Product Information", type="secondary"):
        st.info(
            "💡 You can update your product information in the Chat section with Exporo!"
        )
...
def display_country_selector():
    ...
    if st.session_state.selected_country:
        ...
        with col3:
            ...
            cert_count = len(
                CERTIFICATION_REQUIREMENTS.get(country["code"], {}).get(
                    product_category, []
                )
            )
...
def get_certification_requirements(
    country_code: str, product_category: str
) -> List[str]:
...
def save_assessment_to_memory_bot(assessment_results: Dict):
    ...
    if "export_readiness" not in st.session_state.memory_bot:
        st.session_state.memory_bot["export_readiness"] = DEFAULT_EXTRACTED_DATA[
            "export_readiness"
        ].copy()

    # Add target country if not already present
    target_countries = st.session_state.memory_bot["export_readiness"][
        "target_countries"
    ]
...
def analyze_export_readiness() -> Dict:
    ...
    cert_str = (
        ", ".join(certifications)
        if certifications
        else "No specific certifications found"
    )
...
def get_fallback_analysis(
    memory_data: Dict, country: Dict, certifications: List[str]
) -> Dict:
    ...
    if category_scores["regulatory_compliance"] < 80:
        action_items.append(
            f"Research and obtain required certifications for {country['name']}"
        )
    ...
    # Estimate timeline
    timeline_weeks = len(action_items) * 2 + (
        4 if country["difficulty"] == "High" else 2
    )
    ...
    readiness_level = (
        ...
        if overall_score >= 60
        else "Significant Work Required"
    )
    ...
    return {
        ...
        "potential_challenges": [
            f"{country['difficulty']} market entry difficulty",
...
def display_assessment_results(results: Dict):
    ...
    with col1:
        st.metric(
            "🏛️ Regulatory Compliance", f"{categories['regulatory_compliance']}/100"
        )
        st.metric(
            "📋 Documentation Readiness", f"{categories['documentation_readiness']}/100"
        )

    with col2:
        st.metric("🎯 Market Viability", f"{categories['market_viability']}/100")
        st.metric(
            "🏆 Competitive Positioning", f"{categories['competitive_positioning']}/100"
        )
    ...
    timeline = results.get(
        "timeline_estimate", results.get("timeline", "Not specified")
    )
    ...
    if "export_readiness_level" in results:
        ...
        level_color = (
            ...
            if readiness_level == "Needs Preparation"
            else "#e74c3c"
        )
...
def show_export_readiness_page():
    ...
    if st.session_state.selected_country:
        ...
        with col2:
            if st.button(
                "🚀 Analyze Export Readiness", type="primary", use_container_width=True
            ):
